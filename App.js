...
export default function App() {
  ...
  const [draft, setDraft] = useState({ nome: '', categoria: '', preco: '', peso: '' });
  ...
  const addItem = async () => {
    ...
    const novo = { id: Date.now().toString(), ...draft, preco, peso };
    await persist([...itens, novo]);
    setDraft({ nome: '', categoria: '', preco: '', peso: '' });
    setView('list');
  };
  ...
  const ICMS = 0.20, IOF = 0.0638; // exemplo didático
  const imposto = selected ? (selected.preco || 0) * (ICMS + IOF) : 0;
